...
The following parameters can be set using the builder method (prefixed `With`) or Set (prefixed `Set) before calling the manipulation methods:
  ...
  - Object.schema - Optional but useful for Object.Set. Used to determine the new collections (especially structs) to create in the nested object. Defaults to json collections (maps and slices).

2. Once the Object has been successfully initialized, you can begin calling the manipulation methods: Object.Get, Object.Set, Object.Delete, and `bject.ForEach, which will work on the same `Object.source`.

3. Once you are satisfied, you can call the `Object.GetSource` method to retrieve the modified source especially if changed using `Object.Set` or `Object.Delete`.
